...
void MatrixDisplay::drawStatusPage(const DisplayData& data) {
    ...
    // Line 2 (y=16): (display name)    <- Optional display name
    // Line 3 (y=24): 72°F 45% T125     <- Compact sensors if available
    
    uint16_t status_color = getStatusColor(data.webex_status);
    
    // Line 0: Status indicator + status text
    const String line0_key = String("status|") + data.webex_status;
    ...
    // Line 2: Display name (if set)
    const String line2_key = "name|" + data.display_name;
    if (line2_key != last_line_keys[2]) {
        ...
        if (!data.display_name.isEmpty()) {
            ...
            drawCenteredText(LINE_2_Y, name, COLOR_WHITE);
        }
    }
