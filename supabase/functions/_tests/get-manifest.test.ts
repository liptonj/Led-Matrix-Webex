...
Deno.test("get-manifest: esp-web-tools uses merged firmware", () => {
  const parts = [
    {
      path: "https://example.com/1.5.2/firmware-merged.bin",
      offset: 0,
    },
  ];
...
Deno.test("get-manifest: authenticated requests get signed URLs", () => {
  const publicUrl = "https://example.com/firmware/1.5.2/firmware.bin";
  const signedUrl = "https://example.supabase.co/storage/v1/object/sign/firmware/1.5.2/firmware.bin?token=abc123";
...
Deno.test("get-manifest: falls back to latest if target not found", () => {
  ...
  assertEquals(selectedRelease?.version, "1.5.2");
});
...
Deno.test("get-manifest: respects rollout for authenticated OTA requests", () => {
  const release = { version: "1.5.2", rollout_percentage: 50 };
...
Deno.test("get-manifest: merged firmware path format", () => {
  ...
  assertEquals(mergedPath, "1.5.2/firmware-merged.bin");
});
