...
Deno.test("get-firmware: accepts version query parameter", () => {
  const url = new URL("http://localhost/get-firmware?version=1.5.0");
...
Deno.test("get-firmware: query param takes precedence over target version", () => {
  ...
  const queryVersion = "1.5.0";
  const targetVersion = queryVersion || device.target_firmware_version;

  assertEquals(targetVersion, "1.5.0");
});
...
Deno.test("get-firmware: download URL is signed", () => {
  const signedUrl = "https://example.supabase.co/storage/v1/object/sign/firmware/1.5.0/firmware.bin?token=abc123xyz";
...
Deno.test("get-firmware: success response has required fields", () => {
  const response = {
    success: true,
    version: "1.5.0",
    download_url: "https://example.supabase.co/storage/v1/object/sign/firmware/1.5.0/firmware.bin?token=abc123",
    size: 1234567,
    expires_in: 600,
  };
...
Deno.test("get-firmware: version format is semver-like", () => {
  const validVersions = ["1.0.0", "1.5.0", "2.0.0-beta.1", "1.4.4"];
...
Deno.test("get-firmware: uses latest version when no target specified", () => {
  ...
  assertEquals(latest?.version, "1.5.0");
});
