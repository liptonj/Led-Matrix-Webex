...
-- =============================================================================
-- Part 0: Drop ambiguous dual-parameter overload of user_can_access_device
-- =============================================================================
-- The (text, uuid) overload with defaults causes ambiguity when calling either
-- the (text) or (uuid) overload. The individual overloads are sufficient.

DROP FUNCTION IF EXISTS display.user_can_access_device(text, uuid);
  ...
  IF NOT EXISTS (
    ...
    CREATE POLICY "pairings_user_update_uuid" ON display.pairings
      ...
      USING (
        ...
        OR (device_uuid IS NOT NULL AND display.user_can_access_device(target_device_uuid := device_uuid))
      )
      WITH CHECK (
        display.is_admin()
        OR user_uuid = auth.uid()
