...
void WebSetup::handleRoot(AsyncWebServerRequest* request) {
    ...
    String html = R"rawliteral(<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Webex Display Setup</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,sans-serif;background:#1a1a2e;color:#eee;padding:20px}.container{max-width:400px;margin:0 auto}h1{text-align:center;margin-bottom:20px;color:#00bceb}.card{background:#16213e;border-radius:8px;padding:20px;margin-bottom:15px}h2{font-size:1.1em;margin-bottom:15px;color:#00bceb}.form-group{margin-bottom:15px}label{display:block;margin-bottom:5px;font-size:.9em;color:#aaa}input,select{width:100%;padding:10px;border:1px solid #333;border-radius:4px;background:#0f0f23;color:#fff}input:focus{border-color:#00bceb;outline:none}.btn{display:block;width:100%;padding:12px;border:none;border-radius:4px;font-size:1em;cursor:pointer;margin-top:10px}.btn-primary{background:#00bceb;color:#000}.btn-primary:hover{background:#00a4d1}.btn-secondary{background:#333;color:#fff}.network-list{max-height:200px;overflow-y:auto;margin:10px 0}.network{padding:10px;background:#0f0f23;border-radius:4px;margin-bottom:5px;cursor:pointer;display:flex;justify-content:space-between}.network:hover{background:#1a1a3e}.progress{height:20px;background:#333;border-radius:10px;overflow:hidden;margin:10px 0}.progress-bar{height:100%;background:#00bceb;transition:width .3s}.status{text-align:center;padding:10px;font-size:.9em;color:#aaa}.collapse{display:none}.collapse.show{display:block}.toggle{color:#00bceb;cursor:pointer;font-size:.9em}</style></head><body><div class="container"><h1>Webex Display Setup</h1><div class="card"><h2>WiFi Configuration</h2><button class="btn btn-secondary" onclick="scanNetworks()">Scan Networks</button><div id="networks" class="network-list"></div><form id="wifi-form" onsubmit="saveWifi(event)"><div class="form-group"><label>SSID</label><input type="text" id="ssid" required></div><div class="form-group"><label>Password</label><input type="password" id="password"></div><button type="submit" class="btn btn-primary">Connect</button></form></div><div class="card"><h2>Firmware Update</h2><button class="btn btn-secondary" onclick="loadReleases()">Check for Updates</button><div class="form-group" style="margin-top:10px"><label>Select Version</label><select id="release-select" style="width:100%;padding:10px;background:#0f0f23;color:#fff;border:1px solid #333;border-radius:4px"><option value="-1">Latest Stable (Auto)</option></select></div><div id="ota-status" class="status">Ready to install firmware</div><div class="progress"><div id="progress-bar" class="progress-bar" style="width:0%"></div></div><button class="btn btn-primary" onclick="installSelected()">Install Selected</button><p class="toggle" onclick="toggleAdvanced()">Advanced Options</p><div id="advanced" class="collapse"><div class="form-group"><label>Custom OTA URL (optional)</label><input type="text" id="ota-url" placeholder="Leave empty for default"></div><button class="btn btn-secondary" onclick="saveOTAUrl()">Save URL</button></div><div class="form-group" style="margin-top:15px"><label>Manual Firmware Upload (.bin or bundle)</label><input type="file" id="manual-file" accept=".bin"><button class="btn btn-secondary" onclick="startManualUpload()" id="manual-upload-btn" disabled>Upload Firmware</button><div id="manual-upload-status" class="status">Select a firmware or OTA bundle file to upload</div></div><div class="form-group" style="margin-top:15px"><label>Manual LittleFS Upload (.bin)</label><input type="file" id="manual-fs-file" accept=".bin"><button class="btn btn-secondary" onclick="startManualFsUpload()" id="manual-fs-upload-btn" disabled>Upload LittleFS</button><div id="manual-fs-upload-status" class="status">Select a filesystem image to upload</div></div></div><div class="card"><h2>Status</h2><div id="device-status" class="status">Loading...</div></div></div><script>var scannedNetworks=[],isWifiConnected=false;function scanNetworks(){document.getElementById("networks").innerHTML='<div style="text-align:center;padding:20px">Scanning...</div>',fetch("/api/scan").then(function(e){if(!e.ok)throw new Error("HTTP "+e.status);return e.json()}).then(function(e){scannedNetworks=e.networks||[];for(var t="",n=0;n<scannedNetworks.length;n++){var a=scannedNetworks[n];a.ssid&&(t+='<div class="network" onclick="selectNetwork('+n+')"><span>'+a.ssid+"</span><span>"+a.rssi+"dBm "+(a.encrypted?"&#128274;":"")+"</span></div>")}document.getElementById("networks").innerHTML=t||'<div style="text-align:center;padding:10px">No networks found</div>'}).catch(function(){document.getElementById("networks").innerHTML='<div style="text-align:center;padding:10px;color:#ff6b6b">Scan failed</div>'})}function selectNetwork(e){document.getElementById("ssid").value=scannedNetworks[e].ssid}function saveWifi(e){e.preventDefault();const t=document.getElementById("ssid").value,n=document.getElementById("password").value;fetch("/api/wifi",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ssid:t,password:n})}).then(e=>e.json()).then(e=>{alert(e.message||"WiFi saved! Connecting...")}).catch(()=>alert("Failed to save WiFi"))}function loadReleases(){if(!isWifiConnected)return void(document.getElementById("ota-status").textContent="Connect to WiFi to load versions");document.getElementById("ota-status").textContent="Loading versions from GitHub...";const e=new AbortController,t=setTimeout(()=>e.abort(),15e3);fetch("/api/releases",{signal:e.signal}).then(e=>{if(clearTimeout(t),!e.ok)throw new Error("HTTP "+e.status);return e.json()}).then(e=>{const t=document.getElementById("release-select");if(t.innerHTML='<option value="-1">Latest Stable (Auto)</option>',!e.cached&&e.error)return void(document.getElementById("ota-status").textContent=e.error);e.releases&&e.releases.length>0?(e.releases.forEach(e=>{const n=e.is_beta?" [BETA]":"",a=document.createElement("option");a.value=e.index,a.textContent=e.version+n,e.is_beta&&(a.style.color="#ffcc00"),t.appendChild(a)}),document.getElementById("ota-status").textContent="Found "+e.count+" versions - select and install"):document.getElementById("ota-status").textContent="No releases found (use Latest Stable)"}).catch(e=>{clearTimeout(t),document.getElementById("ota-status").textContent="Error: "+("AbortError"===e.name?"Request timeout":""+e.message)})}function installSelected(){const e=parseInt(document.getElementById("release-select").value);document.getElementById("ota-status").textContent="Starting update...",e>=0?fetch("/api/install-release",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({index:e})}).then(e=>e.json()).then(e=>{e.success?pollProgress():document.getElementById("ota-status").textContent=e.error||"Failed"}):fetch("/api/start-ota",{method:"POST"}).then(e=>e.json()).then(e=>{e.success?pollProgress():document.getElementById("ota-status").textContent=e.error||"Failed"})}function pollProgress(){fetch("/api/ota-progress").then(e=>e.json()).then(e=>{document.getElementById("ota-status").textContent=e.message,document.getElementById("progress-bar").style.width=e.progress+"%",e.progress<100&&"error"!==e.status&&setTimeout(pollProgress,500)})}function saveOTAUrl(){const e=document.getElementById("ota-url").value;fetch("/api/ota-url",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url:e})}).then(e=>e.json()).then(e=>alert(e.message||"URL saved"))}function toggleAdvanced(){document.getElementById("advanced").classList.toggle("show")}function loadStatus(){fetch("/api/status").then(e=>e.json()).then(e=>{let t=`WiFi: ${e.wifi_connected?"Connected":"Disconnected"}<br>`;t+=`IP: ${e.ip_address}<br>`,t+=`Version: ${e.version}<br>`,t+=`Build ID: ${e.build||"--"}`,document.getElementById("device-status").innerHTML=t,isWifiConnected=!!e.wifi_connected})}function initManualUpload(){const e=document.getElementById("manual-file"),t=document.getElementById("manual-upload-btn"),n=document.getElementById("manual-upload-status");e&&t&&n&&e.addEventListener("change",()=>{const a=e.files&&e.files.length>0;t.disabled=!a,n.textContent=a?"Ready to upload.":"Select a firmware or OTA bundle file to upload"})}function initManualFsUpload(){const e=document.getElementById("manual-fs-file"),t=document.getElementById("manual-fs-upload-btn"),n=document.getElementById("manual-fs-upload-status");e&&t&&n&&e.addEventListener("change",()=>{const a=e.files&&e.files.length>0;t.disabled=!a,n.textContent=a?"Ready to upload.":"Select a filesystem image to upload"})}function startManualUpload(){const e=document.getElementById("manual-file"),t=document.getElementById("manual-upload-btn"),n=document.getElementById("manual-upload-status");if(!e||!t||!n||!e.files||0===e.files.length)return void(n&&(n.textContent="No file selected"));if(!confirm("Upload firmware or OTA bundle file? The device will restart when complete."))return;const a=e.files[0];t.disabled=!0,n.textContent="Uploading...";const o=new XMLHttpRequest;o.open("POST","/api/ota/upload"),o.setRequestHeader("Content-Type","application/octet-stream"),o.upload.onprogress=e=>{if(!e.lengthComputable)return;const t=Math.round(e.loaded/e.total*100);n.textContent="Uploading... "+t+"%"},o.onload=()=>{let e="Upload complete. Rebooting...",a=o.status>=200&&o.status<300;if(o.responseText)try{const t=JSON.parse(o.responseText);"boolean"==typeof t.success&&(a=t.success),e=t.message||e}catch(e){}n.textContent=e,a||(t.disabled=!1)},o.onerror=()=>{n.textContent="Upload failed. Please try again.",t.disabled=!1},o.send(a)}function startManualFsUpload(){const e=document.getElementById("manual-fs-file"),t=document.getElementById("manual-fs-upload-btn"),n=document.getElementById("manual-fs-upload-status");if(!e||!t||!n||!e.files||0===e.files.length)return void(n&&(n.textContent="No file selected"));if(!confirm("Upload LittleFS image? The device will restart when complete."))return;const a=e.files[0];t.disabled=!0,n.textContent="Uploading...";const o=new XMLHttpRequest;o.open("POST","/api/ota/upload-fs"),o.setRequestHeader("Content-Type","application/octet-stream"),o.upload.onprogress=e=>{if(!e.lengthComputable)return;const t=Math.round(e.loaded/e.total*100);n.textContent="Uploading... "+t+"%"},o.onload=()=>{let e="Upload complete. Rebooting...",a=o.status>=200&&o.status<300;if(o.responseText)try{const t=JSON.parse(o.responseText);"boolean"==typeof t.success&&(a=t.success),e=t.message||e}catch(e){}n.textContent=e,a||(t.disabled=!1)},o.onerror=()=>{n.textContent="Upload failed. Please try again.",t.disabled=!1},o.send(a)}loadStatus(),setInterval(loadStatus,5e3),initManualUpload(),initManualFsUpload(),setTimeout(loadReleases,1e3);</script></body></html>)rawliteral";
