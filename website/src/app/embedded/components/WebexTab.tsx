...
export const WebexTab = memo(function WebexTab({ isLoggedIn, webexReady, displayName, webexError, webexOauthStatus, webexNeedsAuth, webexPollIntervalMs, onWebexPollIntervalChange, onStartWebexOAuth }: WebexTabProps) {
  return (
    <Card>
      ...
      <div className="mb-4"><Button variant="primary" onClick={onStartWebexOAuth} disabled={!isLoggedIn || webexOauthStatus === 'starting'}>{webexOauthStatus === 'starting' ? 'Starting...' : 'Connect Webex Account'}</Button><p className="text-xs text-[var(--color-text-muted)] mt-2">Opens Webex authorization in a new tab.</p></div>
